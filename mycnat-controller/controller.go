...
import (
	...
	cnatv1alpha1 "github.com/ypxns/k8s_component/mycnat-controller/pkg/apis/cnat/v1alpha1"
	clientset "github.com/ypxns/k8s_component/mycnat-controller/pkg/generated/clientset/versioned"
	cnatscheme "github.com/ypxns/k8s_component/mycnat-controller/pkg/generated/clientset/versioned/scheme"
	informers "github.com/ypxns/k8s_component/mycnat-controller/pkg/generated/informers/externalversions/cnat/v1alpha1"
	listers "github.com/ypxns/k8s_component/mycnat-controller/pkg/generated/listers/cnat/v1alpha1"
)

const controllerAgentName = "mycnat-controller"
...
func NewController(
	...
	// Add mycnat-controller types to the default Kubernetes Scheme so Events can be
	// logged for mycant-controller types.
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Info("Starting mycant client-go controller")

	// Wait for the caches to be synced before starting workers
	klog.Info("Waiting for informer caches to sync")
	if ok := cache.WaitForCacheSync(stopCh, c.podSynced, c.atsSynced); !ok {
...
func (c *Controller) syncHandler(key string) (time.Duration, error) {
	...
	// Clone because the origianl object is owned by the lister.
	instance := original.DeepCopy()

	// If not phase set, default to pending (the initial phase)
	...
	case cnatv1alpha1.PhasePending:
		...
		// As long as we haven't executed the command yet, we need to check if it's time already
		// to act
		klog.Infof("instance %s: checking shedule %q", key, instance.Spec.Schedule)
		...
		if d > 0 {
			// Not yet time to execute the command, wait until the sheduled time
			return d, nil
		}
	...
	case cnatv1alpha1.PhaseDone:
		klog.Infof("instace %s: phase: DONE", key)
		return time.Duration(0), nil
	default:
...
// enqueuePod a pod and checks that the owner refrence points to an At object.
// It then enqueue this At object.
func (c *Controller) enqueuePod(obj interface{}) {
	...
	if pod, ok = obj.(*corev1.Pod); !ok {
		...
		if !ok {
			utilruntime.HandleError(fmt.Errorf("error decoding pod tombstone, invaid type"))
			return
		}
		klog.V(4).Infof("Recovered delete pod '%s' from tombstone", pod.GetName())
	}
...
// newPodForCR creates a new busybox pod with the same name/namespace as the cr
...
// timeUntilSchedule parses the shedule string and returns the time
// until the schedule.
// when it is overdue, the druation is negative.
func timeUntilSchedule(shedule string) (time.Duration, error) {
	now := time.Now().UTC()
	layout := "2006-01-02T15:04:05Z"

	s, err := time.Parse(layout, shedule)
	if err != nil {
		return time.Duration(0), err
	}
